...
void draw_status(const gm::Snapshot& snapshot, int fps) {
  const std::array<std::string, 5> lines = {
      ...
      "Lines " + std::to_string(snapshot.lines),
      "FPS   " + std::to_string(fps),
      phase_label(snapshot.phase),
  };
  static std::array<std::string, 5> last_lines = {};
  for (int i = 0; i < static_cast<int>(lines.size()); i++) {
    if (last_lines[i] == lines[i]) {
      continue;
    }
    last_lines[i] = lines[i];
    text_fixed(STATUS_TOP + 2 + i * 2, STATUS_LEFT + 1, lines[i], 18);
  }
}
...
void render(int fps) {
  ...
  if (first_render) {
    ...
    window(STATUS_TOP, STATUS_LEFT, 12, 13, "Status");
